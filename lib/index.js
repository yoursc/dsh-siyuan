 ...
 * 稍后才消失（实测同一进程内紧接的 SQL 能看到、下一秒再查就没了）。
 * 所以复核要短轮询几次，否则会把成功的删除报成「删除未生效」。
 */
async function waitUntilBlockGone(api, id, attempts = 8, delayMs = 250) {
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    if (!(await blockStillExists(api, id))) return true
    ...
    },
    {
      ...
      definition: defineTextTool({
        ...
        execute: async (args) => {
          ...
          if (!(await waitUntilBlockGone(api, args.blockId))) throw new Error(`思源返回成功，但块 ${args.blockId} 仍然存在，删除未生效`)
          return `已删除块 ${args.blockId}`
        },
      }),
    },
    {
      ...
      definition: defineTextTool({
        ...
        execute: async (args) => {
          ...
          if (!(await waitUntilBlockGone(api, args.docId))) throw new Error(`思源返回成功，但文档 ${args.docId} 仍然存在，删除未生效`)
          return `已删除文档 ${args.docId}`
        },
      }),
    },
  ]
 ...
 */
export const internals = { parseDateArg, startOfDay, goLayout, renderDailyPath, domToText, stripMarks }

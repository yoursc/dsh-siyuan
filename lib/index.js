...
/** 每组的默认开关状态：只在某个工具**没有**显式记录时兜底（新工具/新装/旧配置文件）。 */
const TOOL_GROUP_DEFAULTS = { read: true, write: false, daily: true, danger: false }

const DEFAULT_CONFIG = {
  baseUrl: DEFAULT_BASE_URL,
  defaultNotebook: '',
  // 工具开关按**工具名**存（`{ "siyuan_search": false }`）。旧版是把组名当键存的
  // （`{ "read": true, "write": false, ... }`），这里继续认（见 isToolEnabled）：
  // 旧配置文件不用改写，从设置页保存一次之后自然换成逐工具的键。
  tools: {},
}
...
function normalizeConfig(raw) {
  ...
  // 只收布尔值：键可以是工具名（新版），也可以是组名（旧版）——两者都在 isToolEnabled 里解析。
  // 不在这里补默认值：默认值取决于工具的**分组**，而分组只有 buildTools 知道；
  // 补齐会在没有工具清单的地方写错值。缺项由 isToolEnabled 兜底。
  const tools = {}
...
/**
 * 某个工具当前是否启用。三级判定，顺序固定：
 * 1. `tools[工具名]`（新版逐工具开关，用户在设置页点过就有）；
 * 2. `tools[组名]`（旧版组开关，迁移期继续兜底）；
 * 3. `TOOL_GROUP_DEFAULTS[组名]`（新装 / 新增工具）。
 *
 * @param {object} config - readConfig() 的结果
 * @param {string} name - 工具名（如 siyuan_search）
 * @param {string} group - 工具所属分组（read/write/daily/danger）
 */
...
/**
 * 校验并规范化设置页填的思源地址。非法就抛（保存与"用草稿探测"共用同一套判据），
 * 否则会出现"探测说连不上、保存却报成功"这种自相矛盾的行为。
 */
...
/**
 * 工具调用路径：只要 token 值，不要元数据。
 *
 * describe() 返回的 configured/source/writable 只有设置页用得到；工具调用如果
 * 走 tokenState，每次调用都要为用不到的元数据多付一次凭据库 describe。
 *
 * env 回退语义（两条路径一致）：只有**凭据服务不存在**时才读进程环境变量；
 * 服务存在但库里没值时不回退 env——真实部署里 env 生效是因为 dsh 凭据库把
 * 环境变量当作只读来源从 resolve() 里返回（来源标 "env"、不可写），不是这里读的。
 * @returns {Promise<string>} token 值；未配置即空串
 */
...
/**
 * 设置页路径：读取 token 与它的来源信息（永不回显值）。
 * env 回退语义见 tokenValue。
 ...
 * 自适应解码响应体。
 *
 * 宿主或中间层可能已经解压（此时响应头仍写 gzip），也可能留下压缩体并把
 * `content-encoding` 丢掉（DSH 0.1.7 + undici 8.11 的全局代理 dispatcher 即
 * 如此，实测该头变成 null 而 body 仍是 gzip）。因此不能只信响应头，也不能
 * 只信 body：**魔数优先**（它描述字节实际是什么），头只用于没有魔数的 br。
 * @param {Buffer} buf - 原始响应体
 * @param {string|null|undefined} contentEncoding - response.headers.get('content-encoding')
...
/** 面向工具调用的包装：每次调用现取配置与 token（只取值，见 tokenValue），并带上调用方的取消信号。 */
 ...
 *
 * @returns {Promise<{id: string, hpath: string, count: number}>} `id` 为空串表示该日记还不存在；
 *   `count` 是同路径下的文档总数——createDocWithMd 非幂等，同一路径可能堆出多篇，
 *   工具只读/写第一篇，所以要把这件事告诉调用方
 */
async function lookupDailyDoc(api, notebook, date) {
  ...
  return { id: list.length > 0 ? list[0] : '', hpath, count: list.length }
}

// ── 工具集 ──────────────────────────────────────────────────────────────────

/** 同一路径堆出多篇日记时的提示（createDocWithMd 非幂等的历史遗留）。count ≤ 1 时为空串。 */
function duplicateDailyNote(count, docId) {
  if (count === undefined || count <= 1) return ''
  return `\n\n（注意：该路径下共有 ${count} 篇日记文档，本次只操作了第一篇 id=${docId}；其余未动。可先清理重复的日记。）`
}
      ...
      definition: defineTextTool({
        ...
        execute: async (args, signal) => {
          ...
          if (format === 'markdown') {
            ...
            if (content === '') throw new Error(`文档 ${args.id} 没有导出到内容：该 id 可能是块 id（请用文档 id，即检索结果里的 rootID），或文档本身为空。`)
            return content
          }
      ...
      definition: defineTextTool({
        ...
        execute: async (args, signal) => {
          ...
          if (action === 'append') {
            ...
            return `已追加到日记 ${resolved.hpath}\n文档 id=${docId}${duplicateDailyNote(resolved.count, docId)}`
          }
          const resolved = await lookupDailyDoc(api, notebook, date)
          ...
          return `# ${resolved.hpath}（文档 id=${resolved.id}）\n\n${text === '' ? '（日记为空，或内容都在折叠块里，可用 siyuan_get_child_blocks 查看结构）' : text}${duplicateDailyNote(resolved.count, resolved.id)}`
        },
      }),
    },
    {
    ...
    },
    {
      ...
      definition: defineTextTool({
        ...
        execute: async (args, signal) => {
          const api = makeApi(signal)
          if (args.confirm !== true) throw new Error('删除是破坏性操作，需要在用户明确要求后传 confirm=true')
          // 先查类型（与 delete_block 的预检对齐）：id 不存在给可读错误，不白等复核；
          // 内容块 id 传进来时 removeDocByID 要么报思源原始错误、要么无动作——
          // 都是等满 6 秒复核才报"未生效"，不如一开始就拦下并指路。
          const type = await blockTypeOf(api, args.docId)
          if (type === '') throw new Error(`文档 ${args.docId} 不存在，请核对 id（检索结果里的 rootID 是文档 id）`)
          if (type !== 'd') {
            throw new Error(`块 ${args.docId} 是内容块（type=${type}），不是整篇文档。删除单个内容块请用 siyuan_delete_block。`)
          }
...
async function buildStatePayload(ctx, allTools) {
  ...
  return {
    ...
    // 全部工具定义 + 当前是否启用：开关是**逐工具**的，客户端直接把这里当表单初值
    // （enabled 已经含"逐工具 → 旧组开关 → 组默认"三级解析，客户端不重复实现一遍）。
...
function mount(ctx) {
    ...
    const syncTools = () => {
      ...
      ctx.logger?.info?.(`[dsh-siyuan] 已注册工具：${enabled.join(', ') || '(无)'}`)
    }
    syncTools()
    ...
    /**
     * 探测类路由（测试连接 / 加载笔记本）用的连接参数：**优先用设置页正在编辑的草稿**
     * （body.baseUrl / body.token），没给才回落到已落盘配置与凭据库。
     * 不这么做的话，"填了新地址、没保存就点测试连接"测的是旧地址，结论会误导人。
     */
    ...
    const handlers = {
      ...
      async updateConfig(body) {
        ...
        if (body.tools !== null && typeof body.tools === 'object') {
          ...
          if (Object.keys(incoming).some((key) => !TOOL_GROUPS.includes(key))) {
            // 逐工具提交（新版设置页会把 17 个工具的完整映射发上来）：先把**旧组名键**
            // 对每个工具的结论固化下来，再叠加本次改动、整层替换。
            // 这样没被提到的工具保持原状（不会因为旧键退场而被"组默认"悄悄改写），
            // 迁移也在这一次写入里完成。
            const settled = {}
            for (const entry of allTools) {
      ...
      async testConnection(body) {
        const { config, token } = await probeConnection(body)
        // 三条探测**并发**执行：串行时最坏 3 × 8s = 24s，客户端 api() 的 15s 兜底超时
        // 会把它拦腰截断（设置页显示"请求超时"，但探测其实还在跑）；并发后最坏 ~8s，
        // 客户端超时才退回纯兜底的位置。Promise.all 保持入参顺序，probes 顺序不变。
        ...
        return {
          ...
          // 回报**实际探测的地址与 token 来源**：页面用的是草稿时，用户要能看出这一点。
          baseUrl: config.baseUrl,
 ...
 */
export const internals = { parseDateArg, startOfDay, goLayout, renderDailyPath, domToText, stripMarks, readConfig, writeConfig, configPath, abortError, sleepAbortable, decodeResponseBody, siyuanFetch, buildStatePayload, normalizeSqlStatement, assertNotebookUsable, isToolEnabled, normalizeConfig, normalizeBaseUrlInput, TOOL_GROUP_DEFAULTS, DEFAULT_CONFIG }

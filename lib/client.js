...
`;
		...
		async function api(method, body) {
			const response = await fetch("/siyuan/api/" + method, {
				method: "POST",
		...
		function SiYuanSection() {
			...
			return e(
				...
				e(
					...
					e("p", { className: "dsy-note" }, "改动在「保存设置」后即时生效：工具会按开关重新注册，不需要重启 dsh。当前共 " + state.toolNames.length + " 个工具定义。")
				)
			);
		}
